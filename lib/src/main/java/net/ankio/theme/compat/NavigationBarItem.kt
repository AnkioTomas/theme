...
/**
 * 底部导航栏 Item 兼容层。
 * Material 使用 NavigationBarItem（composable icon/label），
 * Miuix 使用其自有 NavigationBarItem（ImageVector icon + String label）。
 *
 * @param selected 是否选中
 * @param onClick 点击回调
 * @param icon 图标（选中/未选中由调用方决定传入哪个 ImageVector）
 * @param label 标签文本
 */
@Composable
